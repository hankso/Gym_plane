...
#threads = [] 调试多线程用，随后可删除
#先运行该程序才能运行肌电识别程序，两者通过socket传输指令
...
#和trigger client(trigger命令发出者，也即肌电识别结果)通过socket连接
def connect():
    trigger_server = Server()
    #print(trigger_server.trigger)#调试用，随后可删除
